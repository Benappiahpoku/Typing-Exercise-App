  ...
  keys.forEach((key) => {
    ...
    if (firstSpanElement) {
      ...
      if (keyText === event.key.toLowerCase()) {
        firstSpanElement.parentElement.style.backgroundColor = "red";
      }
    }
  });
};

const handlePhysicalKeyRelease = (event) => {
  keys.forEach((key) => {
    const firstSpanElement = key.querySelector("span:first-child");
    if (firstSpanElement) {
      const keyText = firstSpanElement.innerText.toLowerCase();
      if (keyText === event.key.toLowerCase()) {
        firstSpanElement.parentElement.style.backgroundColor = "#445760";
      }
    }
  });
};
    ...
    window.addEventListener("keyup", handlePhysicalKeyRelease);

    return () => {
      window.removeEventListener("keydown", handlePhysicalKeyPress);
      window.removeEventListener("keyup", handlePhysicalKeyRelease);
    };
  }, [inputText]); // Add dependency on inputText
